...
export default function Gallery({ initialCategory = 'All work' }: { initialCategory?: PortfolioView }) {
  ...
  function photoCard(photo: Photograph, position: number, layoutClass = '') {
    const absoluteIndex = fixedSelection ? position : page * PAGE_SIZE + position;
    const orientationClass = photo.width >= photo.height ? 'selected-landscape' : 'selected-portrait';
    return <figure className={`photograph ${layoutClass ? `${layoutClass} ${orientationClass}` : ''}`.trim()} key={photo.id}>
      <DialogTrigger
