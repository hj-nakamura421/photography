...
/** @template {{category: string, monochrome: boolean, width: number, height: number, selected?: boolean}} T
 * @param {T[]} photos
...
export function filterPhotographs(photos, colour = 'all', format = 'all', category = 'All work') {
  return photos.filter(photo =>
    ((category === 'Selected work' && photo.selected === true) ||
      (category === 'All work' && photo.category !== 'School' && photo.category !== '25') ||
...
/** @template T @param {T[]} photos @param {() => number} random @returns {T[]} */
export function shufflePhotographs(photos, random = Math.random) {
